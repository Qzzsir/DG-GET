...
def continus_mixup_data(*xs, y=None,site = None, alpha=1.0, device='cpu'):
    ...
    if site is not None:
        site = lam * site + (1 - lam) * site[index]

    return *new_xs, y, site
...
def mixup_cluster_loss(matrixs, y, intra_weight=2):
    ...
    if sum_0 > 0 and sum_1 > 0:
        loss += intra_weight * (1 - torch.norm(center_0 - center_1, p=1) / (roi_num * roi_num))
